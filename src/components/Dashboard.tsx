...
export default function Dashboard({ 
  ...
  const [branches, setBranches] = useState<any[]>([
    { id: 'BR-01', name: 'Mumbai Main Branch', city: 'Mumbai', address: 'Opera House, Charni Road', phone: '+91 22 2345 6789', staffCount: 0, ledgerAmount: 0, status: 'Open' },
    { id: 'BR-02', name: 'Pune Sub Branch', city: 'Pune', address: 'FC Road, Shivaji Nagar', phone: '+91 20 2567 8901', staffCount: 0, ledgerAmount: 0, status: 'Open' },
    { id: 'BR-03', name: 'Secure Vault Facility', city: 'Mumbai', address: 'Fort Financial District', phone: '+91 22 4001 0022', staffCount: 0, ledgerAmount: 0, status: 'Restricted' },
  ]);
  ...
  const handleCreateLoan = async (e: React.FormEvent) => {
    ...
    // Also add to pledged items vault drawer
    const newPledgedItem = {
      id: `PLG-${loanId.split('-').pop()}`,
      description: newLoanForm.pledgedItem || 'Gold jewelry packet',
      weight: `${newLoanForm.weight} gm`,
      purity: newLoanForm.purity,
      customer: newLoanForm.customerName,
      locker: `Locker A-${Math.floor(10 + Math.random() * 90)}`,
      value: parseFloat(newLoanForm.amount) * 1.35,
      status: 'Secured'
    };
    setPledgedItems(prev => [newPledgedItem, ...prev]);
  ...
  const totalCustomers = Array.from(new Set(loans.map(l => l.customerName))).length;
  const totalGoldWeight = loans.reduce((acc, curr) => curr.status !== 'Closed' ? acc + curr.weight : acc, 0) / 1000; // conversion to kg
  const totalOutstanding = loans.reduce((acc, curr) => curr.status === 'Overdue' ? acc + curr.amount : acc, 0);

  const collectionsToday = payments.reduce((acc, curr) => acc + curr.amount, 0);
  const collectionGoal = 100000; // Daily target ₹ 1 Lakh
  const goalPercentage = collectionGoal > 0 ? Math.min(100, Math.round((collectionsToday / collectionGoal) * 100)) : 0;

  const interestCollected = payments.filter(p => p.type === 'Interest').reduce((acc, curr) => acc + curr.amount, 0);
  const principalRepaid = payments.filter(p => p.type === 'Principal').reduce((acc, curr) => acc + curr.amount, 0);
  const totalCollected = interestCollected + principalRepaid;
  const interestPct = totalCollected > 0 ? Math.round((interestCollected / totalCollected) * 100) : 0;
  const principalPct = totalCollected > 0 ? Math.round((principalRepaid / totalCollected) * 100) : 0;
  ...
  const closedPct = totalLoansCount > 0 ? (closedLoansCount / totalLoansCount) : 0;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
              ...
              >
                ...
                <div className="space-y-2 text-left z-10">
                  ...
                  <h3 className="text-xl md:text-2xl font-black text-[#0A1A36] tracking-tight">{formatINR(totalLoanPortfolio)}</h3>
              ...
              >
                ...
                <div className="space-y-2 text-left z-10">
                  ...
                  <h3 className="text-xl md:text-2xl font-black text-[#0A1A36] tracking-tight">{formatINR(totalOutstanding)}</h3>
                ...
                <div className="relative w-16 h-16 shrink-0 flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle cx="32" cy="32" r="26" stroke="#DF9F28" strokeWidth="5.5" fill="transparent" 
                            strokeDasharray="163" strokeDashoffset={163 - (163 * (goalPercentage / 100))} strokeLinecap="round" />
                  </svg>
                  <div className="absolute text-[10px] font-black text-amber-400">{goalPercentage}%</div>
                </div>

                <div className="space-y-1">
                  <span className="text-3xl font-black tracking-tight text-white block leading-none">{formatINR(collectionsToday)}</span>
                  <p className="text-[10px] text-emerald-400 font-extrabold flex items-center gap-1">
                    <span>{goalPercentage}%</span>
                    <span className="text-slate-400 font-normal">of {formatINR(collectionGoal)} Target</span>
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-2 bg-white/5 p-3 rounded-2xl border border-white/5">
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-bold text-amber-400">{formatINR(interestCollected)} ({interestPct}%)</span>
                    </div>
                    <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-amber-400 h-full rounded-full" style={{ width: `${interestPct}%` }}></div>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-400 font-medium">Principal Repaid</span>
                      <span className="font-bold text-blue-400">{formatINR(principalRepaid)} ({principalPct}%)</span>
                    </div>
                    <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-blue-400 h-full rounded-full" style={{ width: `${principalPct}%` }}></div>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200/60 shadow-[0_4px_20px_-4px_rgba(15,23,42,0.05)] text-left space-y-5 flex flex-col justify-between">
            ...
            <div className="relative flex items-center justify-center py-2">
              <svg className="w-36 h-36 transform -rotate-90">
                ...
                <circle cx="72" cy="72" r="54" stroke="#00A896" strokeWidth="15" fill="transparent" 
                        strokeDasharray="339.3" strokeDashoffset={339.3 - (339.3 * activePct)} strokeLinecap="round" />
                {/* Overdue Segment */}
                <circle cx="72" cy="72" r="54" stroke="#DF9F28" strokeWidth="15" fill="transparent" 
                        strokeDasharray="339.3" strokeDashoffset={339.3 - (339.3 * overduePct)} transform={`rotate(${activeAngle} 72 72)`} strokeLinecap="round" />
                {/* Closed Segment */}
                <circle cx="72" cy="72" r="54" stroke="#3B82F6" strokeWidth="15" fill="transparent" 
                        strokeDasharray="339.3" strokeDashoffset={339.3 - (339.3 * closedPct)} transform={`rotate(${overdueAngle} 72 72)`} strokeLinecap="round" />
              </svg>

              <div className="absolute flex flex-col items-center">
                <span className="text-xl font-black text-[#0A1A36] tracking-tight">{loans.filter(l => l.status !== 'Closed').length}</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-600 bg-slate-50 p-3 rounded-2xl border border-slate-100">
              <div className="flex items-center gap-1.5">
                ...
                <span>Active: {totalLoansCount > 0 ? (activePct * 100).toFixed(1) : 0}%</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 bg-[#DF9F28] rounded-md"></span>
                <span>Overdue: {totalLoansCount > 0 ? (overduePct * 100).toFixed(1) : 0}%</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 bg-[#3B82F6] rounded-md"></span>
                <span>Closed: {totalLoansCount > 0 ? (closedPct * 100).toFixed(1) : 0}%</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 bg-rose-500  rounded-md"></span>
                <span>Auction: 0.0%</span>
              </div>
            </div>
          </div>
