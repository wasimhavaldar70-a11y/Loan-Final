...
export default function InteractiveDevicesMockup() {
  ...
  const activeCustomerLoans = activeCustomer ? loans.filter(l => l.customerId === activeCustomer.id) : [];
                ...
                <div className="flex-1 bg-slate-50 text-slate-800 flex flex-col justify-between overflow-y-auto text-left">
                  ...
                  <div className="flex-1 p-3.5 space-y-3">
                    ...
                    <div className="flex items-center gap-2.5 bg-white p-2 rounded-xl shadow-xs border border-slate-100">
                      ...
                      <div>
                        <h5 className="font-bold text-xs text-slate-800 leading-tight">{activeCustomer?.name || 'No Customer'}</h5>
                        <p className="text-[9px] text-slate-400 font-mono">{activeCustomer?.id || 'N/A'}</p>
                      </div>
                    </div>
                      ...
                      <div className="flex justify-between items-end border-b border-slate-50 pb-2">
                        <div>
                          ...
                          <span className="font-mono text-xs font-bold text-slate-900">
                            ₹{activeCustomer ? activeCustomer.totalLoanAmount.toLocaleString('en-IN') : '0'}
                          </span>
                        </div>
